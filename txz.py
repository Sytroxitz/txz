...
class Position:
    # idx = index; ln = line; col = column; fn = file name; ftxt = file text;
...
class NumberNode:
    def __init__(self, tok): # tok = token
        self.tok = tok
...
class UnaryOpNode: # UnaryOperationNode
...
class ParseResult:
    ...
    def register(self, res): # res = result
...
class Parser:
    ...
    def for_expr(self):
        ...
        if res.error: return res
        if self.current_tok.type != TT_SCOLON:
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected ';'"))
        res.register_advancement()
        self.advance()
    ...
    def expr(self):
        ...
        if res.error: return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'var', 'const', INT, FLOAT, IDENTIFIER, '+', '-', '(' or 'not'"))
        return res.success(node)
...
class Number:
    ...
    def get_comparison_eq(self, other):
        if isinstance(other, Number): return Number(int(self.value == other.value)).set_context(self.context), None
    ...
    def anded_by(self, other):
        if isinstance(other, Number): return Number(int(self.value and other.value)).set_context(self.context), None
    
    def ored_by(self, other):
        if isinstance(other, Number): return Number(int(self.value or other.value)).set_context(self.context), None
...
class Interpreter:
    def visit(self, node, context):
        method_name = f'visit_{type(node).__name__}' # "visit_BinOpNode" / "visit_NumberNode"
